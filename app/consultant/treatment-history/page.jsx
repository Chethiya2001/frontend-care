...
const TreatmentHistoryPage = () => {
  const [patientNic, setPatientNic] = useState(""); // State for search input
  const [treatmentHistory, setTreatmentHistory] = useState([]); // State for treatments
  ...
  // Fetch treatments by patient NIC
  const handleSearch = async () => {
    if (!patientNic.trim()) {
      alert("Please enter a valid Patient NIC.");
      return;
    }

    try {
      const response = await axios.get(
        `http://localhost:5001/treatment/patient/${patientNic}`
      );
      setTreatmentHistory(response.data); // Update table data
    } catch (error) {
      alert(`Error fetching treatments: ${error.response?.data?.message || error.message}`);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="min-h-screen bg-white text-black p-8">
        ...
        <div className="mb-6">
          <label className="block mb-2 text-lg font-semibold">Search by Patient NIC:</label>
          <div className="flex">
            <input
              type="text"
              value={patientNic}
            ...
            <tbody>
              {treatmentHistory.map((treatment) => (
                <tr key={treatment.id} className="hover:bg-gray-100">
                  ...
                  <td className="border border-black p-2">{treatment.patient_name}</td>
                  <td className="border border-black p-2">{treatment.prescribe}</td>
                  <td className="border border-black p-2">{treatment.medicine_discription}</td>
                  <td className="border border-black p-2">{treatment.Illness_description}</td>
                  <td className="border border-black p-2">{treatment.treatment_type_discription}</td>
